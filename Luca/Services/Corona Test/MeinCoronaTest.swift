    ...
    static func decodeTestCode(parse code: String) -> Single<CoronaTest> {
        Single.create { observer -> Disposable in
            do {
                ...
                let publicKeyPath = Bundle.main.url(forResource: "test_meincoronatest_jwtRS256", withExtension: "key.pub")!
                ...
                let jwt = try JWT<MeinCoronaTestClaims>(jwtString: parameters, verifier: verifier)
                observer(.success(MeinCoronaTest(claims: jwt.claims, originalCode: parameters)))
            } catch let error {
