...
class LucaLocationPermissionWorkflow {
    ...
    static func tryToAcquireLocationPermissionAlways(alertToShow infoAlert: Completable = Completable.empty()) -> Single<CLAuthorizationStatus> {
        return LocationPermissionHandler.shared
            .permissionChanges
            .take(1)
            .flatMap { (currentStatus: CLAuthorizationStatus) -> Observable<CLAuthorizationStatus> in
                if currentStatus == .notDetermined {
                    return LocationPermissionHandler.shared.request(.authorizedWhenInUse)
