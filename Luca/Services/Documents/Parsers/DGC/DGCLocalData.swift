...
struct DGCLocalData: Codable {
  static let appVersion = (Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String) ?? "?.?.?"
  static var sharedInstance = DGCLocalData()
  ...
  static let storage = SecureStorage<DGCLocalData>()

  static func initialize(completion: @escaping () -> Void) {
    storage.loadOverride(fallback: DGCLocalData.sharedInstance) { success in
      ...
      if result.lastLaunchedAppVersion != Self.appVersion {
        result.config = DGCLocalData.sharedInstance.config
      }
      DGCLocalData.sharedInstance = result
      completion()
      DGCGatewayConnection.fetchContext()
    }
...
class LocalDataDelegate: PublicKeyStorageDelegate {
  func getEncodedPublicKeys(for kidStr: String) -> [String] {
    DGCLocalData.sharedInstance.encodedPublicKeys[kidStr] ?? []
  }
